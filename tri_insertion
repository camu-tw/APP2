def insertion_tri_score(L, p):
    """
    Trie une liste de dictionnaires L selon la clé p (ex: "fuel", "scoring").
    ...
    for i in range(1, n):
        key = L[i]
        j = i - 1
        while j >= 0 and L[j][p] > key[p]:
            L[j + 1] = L[j]
            j -= 1
            a += 1  # Incrémente à chaque comparaison
        L[j + 1] = key
    return L, a
